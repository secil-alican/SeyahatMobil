//kategori:tarihi,doğa,sanat



export const cities = [
  {
    city: "Eskisehir",
    places: [
      {
        placeImage: "https://i.ibb.co/JW1Qv2zp/porsuk-Cay.jpg",
        placeName: "Porsuk Çayı",
        placeDescription:
          "Eskişehir'in simgelerinden biri olan Porsuk Çayı, şehrin ortasından geçen ve çevresindeki keyifli yürüyüş yolları ile tanınan bir akarsudur. Gondol turları ile de turistlerin ilgisini çeker.",
        placeAdress: "Odunpazarı, Eskişehir",
        placeOpeningHours: "Her zaman erişilebilir",
        placeRatings: 4.5,
        placeMap: [
          {
            latitude: 39.776667,
            longitude: 30.521111,
          },
        ],
      },
      {
        placeImage: "https://i.ibb.co/mFzt9jfx/Odunpazar.jpg",
        placeName: "Odunpazarı",
        placeDescription:
          "Odunpazarı, Eskişehir'in tarihi merkezlerinden biridir. Geleneksel Osmanlı mimarisine sahip evleri, renkli caddeleri ve sanat galerileri ile ünlüdür. Ayrıca Eskişehir'in kültürel mirasını yansıtan birçok mekan burada bulunmaktadır.",
        placeAdress: "Şarkiye, Atatürk Blv. No: 37, 26020 Odunpazarı/Eskişehir",
        placeOpeningHours: "Her zaman erişilebilir",
        placeRatings: 4.6,
        placeMap: [
          {
            latitude: 39.764839,
            longitude: 30.516746,
          },
        ],
      },
      {
        placeImage: "https://i.ibb.co/3wTY9dT/masal-Satosu.jpg",
        placeName: "Masal Şatosu",
        placeDescription:
          "Masal Şatosu, Eskişehir'in Sazova Parkı'nda yer alan ve masallardan ilham alınarak inşa edilen renkli ve büyüleyici bir yapıdır. Hem çocuklar hem de yetişkinler için harika bir ziyaret noktasıdır. Şatoda çeşitli masal karakterlerinin heykelleri ve interaktif alanlar bulunmaktadır.",
        placeAdress:
          "Sazova, Bilim Sanat ve Kültür Parkı, Ulusal Egemenlik Blv., 26150 Tepebaşı/Eskişehir",

        placeOpeningHours: "10:00 - 17:15",
        placeRatings: 4.5,
        placeMap: [
          {
            latitude: 39.774493,
            longitude: 30.518679,
          },
        ],
      },
      {
        placeImage: "https://i.ibb.co/VcG4dD2n/balmumu-M-zesi.jpg",
        placeName: "Balmumu Heykeller Müzesi",
        placeDescription:
          "Balmumu Heykeller Müzesi, Eskişehir'in en ilginç müzelerinden biridir. Bu müzede ünlü kişilerin balmumu heykelleri sergilenmektedir. Ziyaretçiler, ünlü siyasetçilerden sanatçılara kadar birçok kişinin heykelini yakından görebilirler.",
        placeAdress: "Akarbaşı, Atatürk Blv. No:43, 26010 Odunpazarı/Eskişehir",
        placeOpeningHours: "09:00 - 17:00",
        placeRatings: 4.6,
        placeMap: [
          {
            latitude: 39.774299,
            longitude: 30.522071,
          },
        ],
      },
    ],
    activities: [
      {
        activityName: "Gondol Turu",
        activityDescription:
          "Porsuk Çayı üzerinde yapılan gondol turları, şehri romantik bir atmosferde keşfetmek isteyenler için harika bir aktivitedir.",
        activityImage: "https://i.ibb.co/Fkh9rStB/gondol-Turu.jpg",
        activityAdress: "Porsuk Çayı, Odunpazarı, Eskişehir",
        activityOpeningHours: "10:00 - 22:00",
        activityRatings: 4.3,
        activityMap: [
          {
            latitude: 39.776667,
            longitude: 30.521111,
          },
        ],
      },
      {
        activityName: "Odunpazarı Evleri Gezisi",
        activityDescription:
          "Tarihi ve kültürel öneme sahip Odunpazarı evleri, restore edilmiş Osmanlı dönemi yapıları ile şehrin dokusunu yansıtır.",
        activityImage: "https://i.ibb.co/m3ZN5BQ/odunpazar-Gezisijpg.jpg",
        activityAdress: "Odunpazarı Mahallesi, Eskişehir",
        activityOpeningHours: "08:00 - 20:00",
        activityRatings: 4.3,
        activityMap: [
          {
            latitude: 39.764839,
            longitude: 30.516746,
          },
        ],
      },
    ],
    localFoods: [
      {
        foodName: "Çibörek",
        foodImage: "https://i.ibb.co/cKBrx2kj/ciborek.jpg",
        foodDescription:
          "Eskişehir'in meşhur yemeği çibörek, ince hamurun içerisine kıymalı harç koyularak kızartılmasıyla hazırlanır. En popüler adreslerden biri olan Papağan Çibörek, lezzetiyle ünlüdür.",
        restaurantRatings: 4.3,
        restaurantOpeningHours: "08:00 - 20:00",
        restaurantAdress: "Papağan Çibörek Evi, Eskişehir",
        restaurantMap: [
          {
            latitude: 39.764234,
            longitude: 30.525642,
          },
        ],
      },
      {
        foodName: "Balaban Köfte",
        foodImage: "https://i.ibb.co/cSjKB1mn/balaban-Kofte.jpg",
        foodDescription:
          "Izgara köftenin üzerine ekmek, yoğurt ve sos eklenerek servis edilen yöresel bir Eskişehir yemeğidir. Lezzetiyle meşhur Abacı Balaban Köfte, bu yemeğin vazgeçilmez adresidir.",
        restaurantRatings: 4.3,
        restaurantOpeningHours: "11:00 - 22:00",
        restaurantAdress: "Abacı Balaban Köfte, Eskişehir",
        restaurantMap: [
          {
            latitude: 39.765781,
            longitude: 30.529076,
          },
        ],
      },
    ],
  },

  {
    city: "Ankara",
    places: [
      {
        placeImage: "https://i.ibb.co/QCjYZyF/anitkabir.jpg",
        placeName: "Anıtkabir",
        placeDescription:
          "Türkiye Cumhuriyeti'nin kurucusu Mustafa Kemal Atatürk'ün anıt mezarı olan Anıtkabir, tarih ve anlam dolu bir ziyaret noktasıdır.",
        placeAdress: "Mebusevleri, Anıttepe, Anıtkabir, 06570 Çankaya/Ankara",
        placeWeather:
          "Ilıman karasal iklim, yazları sıcak ve kurak, kışları soğuk ve karlı olabilir.",
        placeOpeningHours: "09:00–16:30",
        placeRatings: 4.9,
        placeMap: [
          {
            latitude: 39.925533,
            longitude: 32.837376,
          },
        ],
      },
      {
        placeImage: "https://i.ibb.co/LdJwxCxq/hamamonu-1.jpg",
        placeName: "Hamamönü",
        placeDescription:
          "Tarihi dokusuyla ünlü Hamamönü, restore edilmiş Osmanlı dönemi evleri, sanat galerileri ve kafeleri ile keyifli bir gezi noktasıdır.",
        placeAdress: "Hacettepe, Sarıkadı Sk. No:31, 06230 Altındağ/Ankara",
        placeWeather: "Ilıman karasal iklim, kışları soğuk ve karlı olabilir.",
        placeOpeningHours: "Her zaman erişilebilir",
        placeRatings: 4.6,
        placeMap: [
          {
            latitude: 39.9319390790054,
            longitude: 32.86707166662884,
          },
        ],
      },
      {
        placeImage: "https://i.ibb.co/1GB9jt5m/ankarakalesi-1606815710.jpg",
        placeName: "Ankara Kalesi",
        placeDescription:
          "Ankara'nın en eski yapılarından biri olan Ankara Kalesi, muhteşem şehir manzarası sunan bir tarihi yapıdır.",
        placeAdress: "Kale, 06240 Altındağ/Ankara",
        placeWeather: "Yazları sıcak, kışları serin ve yağışlı olabilir.",
        placeOpeningHours: "10:00–20:00",
        placeRatings: 4.4,
        placeMap: [
          {
            latitude: 39.941043,
            longitude: 32.865042,
          },
        ],
      },
      {
        placeImage:
          "https://i.ibb.co/MxRPvwmj/604-beypazarigezilecekyerler.jpg",
        placeName: "Beypazarı",
        placeDescription:
          "Tarihi evleri, geleneksel el sanatları ve meşhur Beypazarı kurusu ile ünlü bir Ankara ilçesidir.",
        placeAdress: "Beypazarı, Ankara",
        placeWeather:
          "Ilıman karasal iklim, yazları sıcak, kışları soğuk olabilir.",
        placeOpeningHours: "Her zaman erişilebilir",
        placeRatings: 4.7,
        placeMap: [
          {
            latitude: 40.16834311889667,
            longitude: 31.921818,
          },
        ],
      },
      {
        placeImage: "https://i.ibb.co/ZM5VJkf/estergon.jpg",
        placeName: "Estergon Kalesi",
        placeDescription:
          "Türk ve Osmanlı mimarisinin izlerini taşıyan Estergon Kalesi, muhteşem bir manzara sunan kültürel bir mekandır.",
        placeAdress: "Güçlükaya Mh, Fatih Cd., 06280 Keçiören/Ankara",
        placeWeather: "Yazları sıcak ve kurak, kışları soğuk olabilir.",
        placeOpeningHours: "09:00 - 18:00",
        placeRatings: 4.3,
        placeMap: [
          {
            latitude: 39.98103057695309,
            longitude: 32.866399624849414,
          },
        ],
      },
      {
        placeImage: "https://i.ibb.co/V0DzT3yG/eymir-golu.jpg",
        placeName: "Eymir Gölü",
        placeDescription:
          "Doğa severler ve sporcular için harika bir kaçış noktası olan Eymir Gölü, yürüyüş ve bisiklet yolları ile dikkat çeker.",
        placeAdress: "Oran, 06450 Çankaya/Ankara",
        placeWeather: "Yazları sıcak, kışları serin ve yağışlı olabilir.",
        placeOpeningHours: "Her zaman erişilebilir",
        placeRatings: 4.5,
        placeMap: [
          {
            latitude: 39.828072363382525,
            longitude: 32.804165,
          },
        ],
      },
      {
        placeImage:
          "https://i.ibb.co/39ywcHTH/ankara-kugulu-park-kavaklidere-temiz.jpg",
        placeName: "Kuğulu Park",
        placeDescription:
          "Ankara'nın en bilinen parklarından biri olan Kuğulu Park, doğal güzellikleri ve huzurlu atmosferi ile ünlüdür.",
        placeAdress: "Çankaya Kuğulu, Park, 06690 Çankaya/Ankara",
        placeWeather: "Ilıman karasal iklim, kışları soğuk olabilir.",
        placeOpeningHours: "Her zaman erişilebilir",
        placeRatings: 4.5,
        placeMap: [
          {
            latitude: 39.902115299118755,
            longitude: 32.860133280667185,
          },
        ],
      },
      {
        placeImage: "https://i.ibb.co/NgFV9sgf/Tuz-G-l.jpg",
        placeName: "Tuz Gölü",
        placeDescription:
          "Türkiye'nin ikinci büyük gölü olan Tuz Gölü, doğal güzelliği ve fotoğrafçılık için ideal atmosferiyle dikkat çeker.",
        placeAdress: "Şereflikoçhisar, Ankara",
        placeWeather: "Yazları çok sıcak ve kurak, kışları soğuk olabilir.",
        placeOpeningHours: "Her zaman erişilebilir",
        placeRatings: 4.5,
        placeMap: [
          {
            latitude: 38.79395771587093,
            longitude: 33.34234618226869,
          },
        ],
      },
      {
        placeImage:
          "https://i.ibb.co/qY6ZVsxG/bd6281fb-9c24-426a-8e92-81b06d7c9830-DJI-0024.jpg",
        placeName: "Kızılcahamam",
        placeDescription:
          "Kaplıcaları ve doğal güzellikleriyle ünlü Kızılcahamam, doğa tatili yapmak isteyenler için harika bir yerdir.",
        placeAdress: "Kızılcahamam, Ankara",
        placeWeather: "Serin ve yağışlı olabilir, kışları soğuk geçer.",
        placeOpeningHours: "Her zaman erişilebilir",
        placeRatings: 4.5,
        placeMap: [
          {
            latitude: 40.47026933983901,
            longitude: 32.64996703333058,
          },
        ],
      },
      {
        placeImage: "https://i.ibb.co/8wHdHvB/mogan-golu-ve-mogan-park.jpg",
        placeName: "Mogan Gölü",
        placeDescription:
          "Gölbaşı ilçesinde bulunan Mogan Gölü, yürüyüş yolları, restoranları ve piknik alanları ile keyifli bir doğa alanıdır.",
        placeAdress: "Gölbaşı, Ankara",
        placeWeather: "Ilıman, yazları sıcak, kışları soğuk olabilir.",
        placeOpeningHours: "Her zaman erişilebilir",
        placeRatings: 4.4,
        placeMap: [
          {
            latitude: 39.76033834595202,
            longitude: 32.8121,
          },
        ],
      },
    ],

    activities: [
      {
        activityName: "Anıtkabir Ziyareti",
        activityDescription:
          "Anıtkabir’i ziyaret ederek Türkiye Cumhuriyeti'nin kurucusu Mustafa Kemal Atatürk'ü anabilirsiniz. Müze bölümünde Atatürk'e ait eşyalar ve belgeler de sergilenmektedir.",
        activityImage:
          "https://i.ibb.co/1YJnLkYb/thumbs-b-c-943eea179e582795af10b68855db14b7.jpg",
        activityAdress:
          "Mebusevleri, Anıttepe, Anıtkabir, 06570 Çankaya/Ankara",
        activityOpeningHours: "09:00–16:30",
        activityRatings: 5.0,
        activityMap: [
          {
            latitude: 39.925533,
            longitude: 32.836941,
          },
        ],
      },
      {
        activityName: "Kuğulu Park’ta Yürüyüş",
        activityDescription:
          "Ankara'nın en popüler parklarından biri olan Kuğulu Park'ta doğayla iç içe yürüyüş yapabilir, huzur dolu vakit geçirebilirsiniz.",
        activityImage: "https://i.ibb.co/GfwbLwQt/Ku-ulu-Park-5-533x400.jpg",
        activityAdress: "Kavaklıdere, Tunalı Hilmi Cd., 06680 Çankaya/Ankara",
        activityOpeningHours: "Her zaman açık",
        activityRatings: 4.5,
        activityMap: [
          {
            latitude: 39.902115299118755,
            longitude: 32.860133280667185,
          },
        ],
      },
      {
        activityName: "Beypazarı Tarihi Gezisi",
        activityDescription:
          "Osmanlı dönemine ait tarihi konakları ve çarşıyı gezerek geçmişe yolculuk yapabilirsiniz.",
        activityImage:
          "https://i.ibb.co/GG9LSCh/beypazari-news-Image-resize-800-80.jpg",
        activityAdress: "Beypazarı, Ankara",
        activityOpeningHours: "Her zaman açık",
        activityRatings: 4.7,
        activityMap: [
          {
            latitude: 40.16834311889667,
            longitude: 31.921818,
          },
        ],
      },
      {
        activityName: "Eymir Gölü'nde Bisiklet Turu",
        activityDescription:
          "Doğayla iç içe bir bisiklet turu yaparak göl manzarasının keyfini çıkarabilirsiniz.",
        activityImage: "https://i.ibb.co/LXVjxQzc/eymir-golu.jpg",
        activityAdress: "Eymir Gölü, Ankara",
        activityOpeningHours: "Her zaman açık",
        activityRatings: 4.5,
        activityMap: [
          {
            latitude: 39.828072363382525,
            longitude: 32.804165,
          },
        ],
      },
    ],
    localFoods: [
      {
        foodName: "Ankara Tava",
        foodImage: "https://i.ibb.co/Bm7g5Fj/maxresdefault.jpg",
        foodDescription:
          "Kuzu eti ve pirinç pilavı ile hazırlanan, Ankara'nın en meşhur yemeklerinden biridir.",
        restaurantRatings: 4.2,
        restaurantOpeningHours: "07:30–22:00",
        restaurantAdress:
          "Kızılcaşar, Incek Şht. Savcı Mehmet Selim Kiraz Blv No:282, 06830 Gölbaşı/Ankara",
        restaurantMap: [
          {
            latitude: 39.8219362227969,
            longitude: 32.716499515450145,
          },
        ],
      },
      {
        foodName: "Beypazarı Kurusu",
        foodImage:
          "https://i.ibb.co/392xwk3z/74244d5c-be99-4f94-a3e9-b8776162db59.png",
        foodDescription:
          "Beypazarı'na özgü, incecik açılmış hamurun fırında kurutulmasıyla yapılan ve çeşitli yemeklerin yanında servis edilen bir atıştırmalıktır.",
        restaurantRatings: 4.7,
        restaurantOpeningHours: "07:00–20:30",
        restaurantAdress: "Kurtuluş, Kapan Sok. No:22, 06730 Beypazarı/Ankara",
        restaurantMap: [
          {
            latitude: 40.16805292168806,
            longitude: 31.921122552790642,
          },
        ],
      },
      {
        foodName: "Uruş Kapama",
        foodImage: "https://i.ibb.co/7x8jwM5P/kapama.jpg",
        foodDescription:
          "Özellikle düğünlerde ve özel günlerde yapılan, kuzu eti ile sebzelerin yavaşça pişirilerek kapandığı lezzetli bir yemektir.",
        restaurantRatings: 3.9,
        restaurantOpeningHours: "08:00–00:00",
        restaurantAdress:
          "Cumhuriyet, Beypazarı-Karaşar Yolu, 06730 Beypazarı/Ankara",
        restaurantMap: [
          {
            latitude: 40.19379451805092,
            longitude: 31.90866090386253,
          },
        ],
      },

      {
        foodName: "Beypazarı Güveci",
        foodImage: "https://i.ibb.co/7JVVjJxG/beypazariguveci.jpg",
        foodDescription:
          "Odun ateşinde ağır ağır pişirilen, et ve sebzelerle yapılan geleneksel bir güveç yemeği.",
        restaurantRatings: 4.1,
        restaurantOpeningHours: "08:00–18:00",
        restaurantAdress:
          "Beytepe, Bostancılar Sk. No:34, 06730 Beypazarı/Ankara",
        restaurantMap: [
          {
            latitude: 40.16900694580494,
            longitude: 31.9198684671879,
          },
        ],
      },
      {
        foodName: "Ankara Simidi",
        foodImage: "https://i.ibb.co/hFwdJ8f4/ankara-simidi.jpg",
        foodDescription:
          "Susamla kaplı, çıtır dışı ve yumuşak içi ile ünlü Ankara simidi, kahvaltılarda sıkça tüketilir.",
        restaurantRatings: 4.2,
        restaurantOpeningHours: "07:00 - 19:00",
        restaurantAdress: "Çankaya, Cinnah Cd. No:21, 06680 Çankaya/Ankara",
        restaurantMap: [
          {
            latitude: 39.906172149350695,
            longitude: 32.860049856798994,
          },
        ],
      },
    ],
  },

  {
    city: "İstanbul",
    places: [
      {
        category: "Tarihi",
        placeImage: "https://i.ibb.co/7J22QVm3/ayasofyanin-gizemi.png",
        placeName: "Ayasofya",
        placeDescription:
          "Ayasofya, Bizans ve Osmanlı İmparatorlukları dönemlerinde kilise ve cami olarak kullanılan, günümüzde ise müze ve ibadethane olarak ziyaretçilerini ağırlayan tarihi bir yapıdır.",
        placeAdress:
          "Sultan Ahmet, Ayasofya Meydanı No:1, 34122 Fatih/İstanbul",
        placeOpeningHours: "Her Zaman Erişilebilir",
        placeRatings: 4.8,
        placeMap: [
          {
            latitude: 41.00875298925186,
            longitude: 28.980164267231594,
          },
        ],
      },
      {
        category: "Tarihi",
        placeImage: "https://i.ibb.co/jPhQKNph/top1-620x400.jpg",
        placeName: "Topkapı Sarayı",
        placeDescription:
          "Osmanlı İmparatorluğu’nun yönetim merkezi olarak kullanılan Topkapı Sarayı, ihtişamlı mimarisi ve Osmanlı tarihine dair önemli eserleri ile dikkat çekmektedir.",
        placeAdress: "Cankurtaran, 34122 Fatih/İstanbul",
        placeOpeningHours: "09:00–17:00",
        placeRatings: 4.6,
        placeMap: [
          {
            latitude: 41.01169113848889,
            longitude: 28.98340625058665,
          },
        ],
      },
      {
        category: "Tarihi",
        placeImage: "https://i.ibb.co/jCqbnwX/blue-mosque-15cbc679cd9d0e.jpg",
        placeName: "Sultanahmet Camii",
        placeDescription:
          "Mavi Camii olarak da bilinen Sultanahmet Camii, Osmanlı dönemine ait en önemli eserlerden biridir. Muhteşem iç dekorasyonu ve altı minaresiyle ünlüdür.",
        placeAdress: "Sultan Ahmet, 34122 Fatih/İstanbul",
        placeOpeningHours: "Her Zaman Erişilebilir",
        placeRatings: 4.7,
        placeMap: [
          {
            latitude: 41.00592682499263,
            longitude: 28.977309340245295,
          },
        ],
      },
      {
        category: "Tarihi",
        placeImage:
          "https://i.ibb.co/S7P4DbHk/20231124-kapali-carsi-915520-589236045ac5a157d94f.jpg",
        placeName: "Kapalı Çarşı",
        placeDescription:
          "Kapalı Çarşı, İstanbul'un en eski ve en büyük kapalı alışveriş merkezlerinden biridir. Altın, halı, baharat ve hediyelik eşya gibi birçok ürün burada bulunabilir.",
        placeAdress: "Beyazıt, 34126 Fatih/İstanbul",
        placeOpeningHours: "09:00 - 19:00",
        placeRatings: 4.4,
        placeMap: [
          {
            latitude: 41.01085478382827,
            longitude: 28.968068096067423,
          },
        ],
      },
      {
        category: "Tarihi",
        placeImage: "https://i.ibb.co/0VmY6m77/bo-azdan-scaled.jpg",
        placeName: "Dolmabahçe Sarayı",
        placeDescription:
          "Osmanlı İmparatorluğu'nun son dönemlerinde kullanılan Dolmabahçe Sarayı, Avrupa mimarisinden etkilenerek inşa edilmiş ve zarif detayları ile göz kamaştırmaktadır.",
        placeAdress: "Vişnezade, Dolmabahçe Cd., 34357 Beşiktaş/İstanbul",
        placeOpeningHours: "09:00 - 18:00 (Pazartesi Kapalı)",
        placeRatings: 4.7,
        placeMap: [
          {
            latitude: 41.03928438220083,
            longitude: 29.00047107618727,
          },
        ],
      },

      {
        category: "Manzara",
        placeImage: "https://i.ibb.co/G4F8tcmd/istanbul-bopshorus.jpg",
        placeName: "İstanbul Boğazı",
        placeDescription:
          "İstanbul Boğazı, Asya ve Avrupa kıtalarını ayıran doğal bir su yoludur. Muhteşem manzaraları, tarihi yalıları ve köprüleriyle ünlüdür. Boğaz turlarıyla bu eşsiz güzelliği keşfedebilirsiniz.",
        placeAddress: "İstanbul Boğazı, İstanbul",
        placeOpeningHours: "Her zaman Erişilebilir",
        placeRatings: 4.8,
        placeMap: [
          {
            latitude: 41.123,
            longitude: 29.056,
          },
        ],
      },
      {
        category: "Tarihi",
        placeImage: "https://i.ibb.co/sdxqgTD9/galata-kulesi-tarihi.webp",
        placeName: "Galata Kulesi",
        placeDescription:
          "Galata Kulesi, İstanbul'un en ikonik yapılarından biridir. 14. yüzyılda inşa edilen bu kule, şehrin panoramik manzarasını sunar. Tarihi dokusu ve eşsiz manzarasıyla ziyaretçileri cezbeder.",
        placeAddress: "Bereketzade, 34421 Beyoğlu/İstanbul",
        placeOpeningHours: "08:30–22:00",
        placeRatings: 4.6,
        placeMap: [
          {
            latitude: 41.02581988528732,
            longitude: 28.974107138396924,
          },
        ],
      },
      {
        category: "Tarihi",
        placeImage: "https://i.ibb.co/TG95bg5/yerebatan-sarnici-10.jpg",
        placeName: "Yerebatan Sarnıcı",
        placeDescription:
          "Yerebatan Sarnıcı, Bizans döneminden kalma yer altı su deposudur. Sarnıç, etkileyici sütunları ve mistik atmosferiyle ziyaretçilerine eşsiz bir deneyim sunar.",
        placeAddress: "Alemdar, Yerebatan Cd. 1/3, 34110 Fatih/İstanbul",

        placeOpeningHours: "09:00–18:30 | 19:30–22:00",
        placeRatings: 4.6,
        placeMap: [
          {
            latitude: 41.0085863741744,
            longitude: 28.977963826752344,
          },
        ],
      },
      {
        category: "",
        placeImage: "https://i.ibb.co/tpWb3RMn/Sultan-Ahmet-Cover.jpg",
        placeName: "Sultanahmet Meydanı",
        placeDescription:
          "Sultanahmet Meydanı, İstanbul'un tarihi yarımadasında yer alır ve Ayasofya, Sultanahmet Camii ve Topkapı Sarayı gibi önemli yapılarla çevrilidir. Tarihi dokusu ve mimarisiyle dikkat çeker.",
        placeAddress:
          "Binbirdirek, Sultan Ahmet Parkı No:2, 34122 Fatih/İstanbul",
        placeOpeningHours: "Her zaman Erişilebilir",
        placeRatings: 4.7,
        placeMap: [
          {
            latitude: 41.00648825647748,
            longitude: 28.975655152709194,
          },
        ],
      },
      {
        category: "",
        placeImage: "https://i.ibb.co/0yDqHd7s/6257739075-67337fc423-b.jpg",
        placeName: "İstiklal Caddesi",
        placeDescription:
          "İstiklal Caddesi, İstanbul'un en popüler alışveriş ve eğlence caddelerinden biridir. Tarihi binaları, mağazaları, restoranları ve sanat galerileriyle ünlüdür.",
        placeAddress: "Beyoğlu/İstanbul",
        placeOpeningHours: "Her zaman Erişilebilir",
        placeRatings: 4.6,
        placeMap: [
          {
            latitude: 41.03382783635566,
            longitude: 28.977952696068616,
          },
        ],
      },
      {
        category: "",
        placeImage: "https://i.ibb.co/KjXRG2M9/1200x628.jpg",
        placeName: "Taksim Meydanı",
        placeDescription:
          "Taksim Meydanı, İstanbul'un en merkezi noktalarından biridir. Alışveriş merkezleri, restoranlar ve otellerle çevrilidir.",
        placeAddress: "Taksim Meydanı, Beyoğlu, İstanbul",
        placeOpeningHours: "Her zaman Erişilebilir",
        placeRatings: 4.5,
        placeMap: [
          { latitude: 41.03768106719055, longitude: 28.985091638323603 },
        ],
      },
      {
        category: "Tarihi",
        placeImage: "https://i.ibb.co/bM9RnjJn/kiz-kulesinin-hikayesi.jpg",
        placeName: "Kız Kulesi",
        placeDescription:
          "Kız Kulesi, İstanbul Boğazı'nda küçük bir adacık üzerinde yer alan tarihi bir kuledir. Günümüzde restoran ve müze olarak hizmet vermektedir.",
        placeAddress: "Salacak, 34668 Üsküdar/İstanbul",
        placeOpeningHours: "09:30–18:00",
        placeRatings: 4.7,
        placeMap: [
          { latitude: 41.021388719934265, longitude: 29.00436799168176 },
        ],
      },
      {
        category: "Tarihi",
        placeImage: "https://i.ibb.co/dJ4PTRMp/misir-carsisi.jpg",
        placeName: "Mısır Çarşısı",
        placeDescription:
          "Mısır Çarşısı, baharatları, kuruyemişleri ve lokumlarıyla ünlü tarihi bir çarşıdır. Renkli atmosferiyle ziyaretçilerine eşsiz bir alışveriş deneyimi sunar.",
        placeAddress: "Rüstem Paşa, 34116 Fatih/İstanbul",
        placeOpeningHours: "08:00–19:30",
        placeRatings: 4.5,
        placeMap: [
          { latitude: 41.016749242987444, longitude: 28.970489984424127 },
        ],
      },
      {
        category: "",
        placeImage: "https://i.ibb.co/5XT5N5DK/607573cbf434d99a1a1a8a23.jpg",
        placeName: "Eminönü",
        placeDescription:
          "Eminönü, İstanbul'un tarihi yarımadasında yer alan ve ticaretin kalbinin attığı bir bölgedir. Balık ekmekçileri, tarihi yapıları ve çarşılarıyla ünlüdür.",
        placeAddress: "Eminönü, Fatih, İstanbul",
        placeOpeningHours: "Her zaman Erişilebilir",
        placeRatings: 4.6,
        placeMap: [
          { latitude: 41.01748449657007, longitude: 28.970921767232042 },
        ],
      },
      {
        category: "Tarihi",
        placeImage:
          "https://i.ibb.co/V0NbdNfG/1200px-S-leymaniye-Mosque-Istanbul-cropped.jpg",
        placeName: "Süleymaniye Camii ve Külliyesi",
        placeDescription:
          "Süleymaniye Camii, Mimar Sinan'ın en önemli eserlerinden biridir ve İstanbul'un siluetine damgasını vurur. Külliye içerisinde medrese, hamam ve imaret gibi yapılar da bulunur.",
        placeAddress:
          "Süleymaniye, Prof. Sıddık Sami Onar Cd. No:1, 34116 Fatih/İstanbul",
        placeOpeningHours: "09:00 - 18:00",
        placeRatings: 4.9,
        placeMap: [
          { latitude: 41.01613601831407, longitude: 28.9640354690814732 },
        ],
      },
      {
        category: "Sanat",
        placeImage: "https://i.ibb.co/JRQd0zDx/koleksiyon-78310-977448.jpg",
        placeName: "İstanbul Modern Sanat Müzesi",
        placeDescription:
          "İstanbul Modern, çağdaş sanat eserleriyle tanınan, Türkiye'nin en önemli modern sanat müzelerinden biridir. Sürekli ve geçici sergilere ev sahipliği yapar.",
        placeAddress:
          "Kılıçali Paşa, Tophane İskele Cd. No:1/1, 34433 Beyoğlu/İstanbul",
        placeOpeningHours: "10:00 - 18:00 (Pazartesi kapalı)",
        placeRatings: 4.4,
        placeMap: [
          { latitude: 41.02604907474936, longitude: 28.982956313260146 },
        ],
      },
      {
        category: "Sanat",
        placeImage:
          "https://i.ibb.co/1Y4C40td/istanbul-arkeoloji-muzesi-gezisi.jpg",
        placeName: "İstanbul Arkeoloji Müzeleri",
        placeDescription:
          "İstanbul Arkeoloji Müzeleri, antik eser koleksiyonlarıyla dünya çapında ünlüdür. Müze, Eski Şark Eserleri Müzesi, Arkeoloji Müzesi ve Çinili Köşk Müzesi olmak üzere üç bölümden oluşur.",
        placeAddress: "Cankurtaran, 34122 Fatih/İstanbul",
        placeOpeningHours: "09:00–17:30",
        placeRatings: 4.6,
        placeMap: [
          { latitude: 41.011725949941074, longitude: 28.98133049606743 },
        ],
      },
      {
        category: "Sanat",
        placeImage: "https://i.ibb.co/xq1mM1WJ/miniaturk-muzesi-istanbul.jpg",
        placeName: "Miniatürk",
        placeDescription:
          "Miniatürk, Türkiye'nin önemli tarihi yapılarının minyatür versiyonlarını sergileyen açık hava müzesidir. Çocuklar ve turistler için eğlenceli bir gezi noktasıdır.",
        placeAddress: "Örnektepe, İmrahor Cd. No:7, 34445 Beyoğlu/İstanbul",
        placeOpeningHours: "09:00 - 18:00",
        placeRatings: 4.6,
        placeMap: [
          { latitude: 41.05913405715795, longitude: 28.949265215111417 },
        ],
      },
      {
        category: "Sanat",
        placeImage:
          "https://i.ibb.co/gbKL6Vcn/istanbul-pera-museum-exterior2.jpg",
        placeName: "Pera Müzesi",
        placeDescription:
          "Pera Müzesi, Osmanlı ve Cumhuriyet dönemine ait sanat koleksiyonlarıyla dikkat çeker. Düzenlenen sergiler ve kültürel etkinliklerle sanatseverleri ağırlar.",
        placeAddress:
          "Asmalı Mescit, Meşrutiyet Cd. No:65, 34430 Beyoğlu/İstanbul",
        placeOpeningHours:
          "10:00–19:00 (Pazartesi kapalı) | Cumartesi : 10:00–19:00",
        placeRatings: 4.6,
        placeMap: [
          { latitude: 41.03200734274797, longitude: 28.97502560956156 },
        ],
      },
      {
        category: "Sanat",
        placeImage: "https://i.ibb.co/844Q0rKr/image.jpg",
        placeName: "Sakıp Sabancı Müzesi",
        placeDescription:
          "Sakıp Sabancı Müzesi, Osmanlı hat sanatları, modern sanat eserleri ve geçici sergileriyle İstanbul’un kültürel hayatında önemli bir yere sahiptir.",
        placeAddress:
          "Emirgan, Sakıp Sabancı Cd. No:42, 34467 Sarıyer/İstanbul",
        placeOpeningHours: "10:00–18:00 (Pazartesi kapalı)",
        placeRatings: 4.7,
        placeMap: [
          { latitude: 41.10568484410979, longitude: 29.05688000166506 },
        ],
      },
    ],
  },
];
